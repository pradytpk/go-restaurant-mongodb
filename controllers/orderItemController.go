...
import (
	...
	"github.com/pradytpk/go-restaurant-management/database"
	"github.com/pradytpk/go-restaurant-management/models"
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	groupStage := bson.D{{Key: "$group", Value: bson.D{{Key: "_id", Value: bson.D{{Key: "order_id", Value: "$order_id"}, {Key: "table_id", Value: "$table_id"}, {Key: "table_number", Value: "$table_number"}}}, {Key: "payment_due", Value: bson.D{{Key: "$sum", Value: "$amount"}}}, {Key: "total_count", Value: bson.D{{"$sum", 1}}}, {Key: "order_items", Value: bson.D{{Key: "$push", Value: "$$ROOT"}}}}}}
